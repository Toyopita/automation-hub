#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用して株式（米国+日本）ニュースを「📈｜株式」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448488252582531214
"""
...
# 📈｜株式 フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448488252582531214

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_stock_news_history.json'

# Google News RSS URL（株式市場関連ニュース - 米国+日本）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=S%26P500 OR ダウ平均 OR NASDAQ OR 米国株 OR ウォール街 OR FRB OR 日経平均 OR TOPIX OR 東証 OR 日本株 OR 株式市場 OR 決算発表 OR 日銀&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSから株式ニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}の株式ニュース {posted_count}件をIdiomに投稿しました" with title "Idiom 株式ニュース"\'')
