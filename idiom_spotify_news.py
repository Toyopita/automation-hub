#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用してSpotifyニュースを「🎵｜Spotify」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448467290344652963
"""
...
# 🎵｜Spotify フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448467290344652963

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_spotify_news_history.json'

# Google News RSS URL（Spotifyに関するニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=Spotify OR スポティファイ OR 音楽ストリーミング OR Apple Music OR Amazon Music&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSからSpotifyニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}のSpotifyニュース {posted_count}件をIdiomに投稿しました" with title "Idiom Spotifyニュース"\'')
