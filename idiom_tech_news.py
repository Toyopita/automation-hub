#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用してテック（半導体+GPU）ニュースを「🔌｜テック」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448488250464407594
"""
...
# 🔌｜テック フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448488250464407594

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_tech_news_history.json'

# Google News RSS URL（半導体+GPU関連ニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=半導体 OR NVIDIA OR AMD OR Intel OR TSMC OR Qualcomm OR ARM OR ASML OR GPU OR GeForce OR Radeon OR RTX OR グラフィックボード OR CUDA&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSからテックニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}のテックニュース {posted_count}件をIdiomに投稿しました" with title "Idiom テックニュース"\'')
