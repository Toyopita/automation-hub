#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用してNotionニュースを「📝｜notionニュース」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448462409370046597
"""
...
# 📝｜notionニュース フォーラムチャンネルID（Idiomサーバー）
NOTION_FORUM_ID = 1448462409370046597

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_notion_news_history.json'
...
async def on_ready():
    ...
    try:
        ...
        # macOS通知
        os.system(f'osascript -e \'display notification "{today}のNotionニュース {posted_count}件をIdiomに投稿しました" with title "Idiom Notionニュース"\'')
