#!/usr/bin/env python3
"""
1時間ごとに大阪関西万博ニュースをIZUMOサーバーの「🎡｜大阪関西万博」フォーラムに投稿
...
# 投稿履歴ファイル
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/expo_news_history.json'

# Google News RSS URL（大阪関西万博ニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=大阪関西万博 OR 大阪万博 OR 関西万博 OR EXPO2025&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        # フォーラムチャンネルを取得
        forum = bot.get_channel(EXPO_FORUM_ID)
        if not forum:
            print(f'エラー: フォーラムチャンネル（ID: {EXPO_FORUM_ID}）が見つかりません')
            await bot.close()
            return
        ...
        # Google News RSSから最新ニュースを取得
        print('Google News RSSから大阪関西万博ニュースを取得中...')
...
{news['url']}"""
        ...
        # macOS通知
        os.system(f'osascript -e \'display notification "{today}の大阪関西万博ニュース {posted_count}件を投稿しました（Google News RSS使用）" with title "Discord万博ニュース"\'')
