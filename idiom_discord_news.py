#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用してDiscordニュースを「💬｜Discord」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448466764668207167
"""
...
# 💬｜Discord フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448466764668207167

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_discord_news_history.json'

# Google News RSS URL（Discordに関するニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=Discord アプリ OR Discord ゲーム OR Discord コミュニティ OR Discord 新機能 OR Discord Bot&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSからDiscordニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}のDiscordニュース {posted_count}件をIdiomに投稿しました" with title "Idiom Discordニュース"\'')
