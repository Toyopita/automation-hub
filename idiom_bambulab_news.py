#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用してBambu Labニュースを「🖨️｜Bambu Lab」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448467288406757458
"""
...
# 🖨️｜Bambu Lab フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448467288406757458

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_bambulab_news_history.json'

# Google News RSS URL（Bambu Labに関するニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=Bambu Lab OR 3Dプリンター OR 3Dプリント OR FDM OR フィラメント&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSからBambu Labニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}のBambu Labニュース {posted_count}件をIdiomに投稿しました" with title "Idiom Bambu Labニュース"\'')
