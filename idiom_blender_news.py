#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用してBlenderニュースを「🎨｜Blender」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448466761635856558
"""
...
# 🎨｜Blender フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448466761635856558

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_blender_news_history.json'

# Google News RSS URL（Blenderに関するニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=Blender 3D OR Blender CG OR Blender アニメーション OR Blender モデリング&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSからBlenderニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}のBlenderニュース {posted_count}件をIdiomに投稿しました" with title "Idiom Blenderニュース"\'')
