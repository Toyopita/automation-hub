...
function sendScheduleToDiscord(events) {
  ...
  const todayEvents = events.filter(e => {
    if (e.calendarName === '六曜カレンダー') return false; // 六曜は除外
...
\`自動送信 | ${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')} ${String(today.getHours()).padStart(2, '0')}:${String(today.getMinutes()).padStart(2, '0')}\``;
  ...
  try {
    ...
    if (responseCode === 204 || responseCode === 200) {
      ...
      Logger.log(`❌ Discord投稿失敗: ${responseCode}`);
...
/**
 * メイン処理にDiscord投稿を追加
 * 既存のnotifyUpcomingTasksAndEvents()関数の最後に以下を追加：
 */
/*
  // 既存のメール送信後に追加
  if (upcomingTasks.length > 0 || upcomingEvents.length > 0) {
    ...
    // Discord投稿を追加
    sendToDiscord(upcomingEvents, upcomingTasks);
  } else {
    Logger.log('通知対象のタスク・予定はありません。');
  }
*/
...
function setDiscordWebhookUrl() {
  const props = PropertiesService.getScriptProperties();
  const WEBHOOK_URL = 'https://discord.com/api/webhooks/1434377710863515698/D1i7mp6Kx4pSxTUpYRLELAjQcy1LmnaDbyc8OpuTrYIUKef4-tMMzJbvWBz1cWUOLgG3';
  props.setProperty('DISCORD_WEBHOOK_URL', WEBHOOK_URL);
  Logger.log('✅ DISCORD_WEBHOOK_URLを設定しました');
}
