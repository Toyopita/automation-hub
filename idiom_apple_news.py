#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用してAppleニュースを「🍎｜Apple」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448466763242143814
"""
...
# 🍎｜Apple フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448466763242143814

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_apple_news_history.json'

# Google News RSS URL（Appleに関するニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=Apple iPhone OR Apple Mac OR Apple iPad OR Apple Watch OR iOS OR macOS OR Apple Intelligence&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSからAppleニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}のAppleニュース {posted_count}件をIdiomに投稿しました" with title "Idiom Appleニュース"\'')
