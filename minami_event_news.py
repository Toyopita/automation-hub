#!/usr/bin/env python3
"""
1時間ごとに関西イベントニュースをMinamiサーバーの「🎪イベント」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1434499089420128317
"""
...
# 🎪イベント フォーラムチャンネルID（Minamiサーバー）
EVENT_FORUM_ID = 1434499089420128317

# 投稿履歴ファイル
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/minami_event_news_history.json'

# Google News RSS URL（関西イベントニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=関西 イベント OR 大阪 イベント OR 京都 イベント OR 神戸 イベント OR 奈良 イベント OR 展覧会 関西 OR フェス 関西 OR コンサート 関西&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        # フォーラムチャンネルを取得
        forum = bot.get_channel(EVENT_FORUM_ID)
        if not forum:
            print(f'エラー: フォーラムチャンネル（ID: {EVENT_FORUM_ID}）が見つかりません')
            await bot.close()
            return
        ...
        # Google News RSSから最新ニュースを取得
        print('Google News RSSから関西イベントニュースを取得中...')
...
{news['url']}"""
        ...
        # macOS通知
        os.system(f'osascript -e \'display notification "{today}の関西イベントニュース {posted_count}件を投稿しました（Google News RSS使用）" with title "Discord関西イベントニュース"\'')
