#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用してGoogleニュースを「🔍｜Google」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448484619220684931
"""
...
# 🔍｜Google フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448484619220684931

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_google_news_history.json'

# Google News RSS URL（Googleに関するニュース）
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=Google Android OR Google Pixel OR Google Chrome OR Gmail OR Google Maps OR YouTube OR Google Cloud&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSからGoogleニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}のGoogleニュース {posted_count}件をIdiomに投稿しました" with title "Idiom Googleニュース"\'')
