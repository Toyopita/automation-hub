...
def main():
    ...
    print(f'   タスク数: {len(GEKKISAI_TASKS)}件\n')

    # タスクを順次作成
    success_count = 0
    for i, task_name in enumerate(GEKKISAI_TASKS, 1):
        print(f'[{i}/{len(GEKKISAI_TASKS)}] {task_name}...', end=' ')
    ...
    # 結果サマリー
    print(f'\n📊 結果: {success_count}/{len(GEKKISAI_TASKS)}件のタスクを作成しました')

    # macOS通知
    if success_count == len(GEKKISAI_TASKS):
        os.system(f'osascript -e \'display notification "月次祭タスク{len(GEKKISAI_TASKS)}件を自動生成しました" with title "祖霊社タスク自動生成"\'')
    elif success_count > 0:
        os.system(f'osascript -e \'display notification "月次祭タスク{success_count}/{len(GEKKISAI_TASKS)}件を生成しました（一部失敗）" with title "祖霊社タスク自動生成"\'')
    else:
