#!/usr/bin/env python3
"""
毎時IdiomサーバーのGoogle News RSSを使用して国際情勢ニュースを「🌍｜国際情勢」フォーラムに投稿

Google News RSSを使用（完全無料・API key不要）
フォーラムチャンネルID: 1448488699297009774
"""
...
# 🌍｜国際情勢 フォーラムチャンネルID（Idiomサーバー）
FORUM_ID = 1448488699297009774

# 投稿履歴ファイル（Idiom専用）
HISTORY_FILE = '/Users/minamitakeshi/discord-mcp-server/idiom_world_news_history.json'

# Google News RSS URL（国際情勢関連ニュース - 網羅的キーワード）
# カテゴリ: 一般、国際機関、地域情勢、国名・関係、軍事・安保、外交用語、人道
GOOGLE_NEWS_RSS = 'https://news.google.com/rss/search?q=国際情勢 OR 外交 OR 首脳会談 OR 国連 OR NATO OR EU OR G7 OR G20 OR ASEAN OR BRICS OR IMF OR WHO OR WTO OR APEC OR AUKUS OR QUAD OR 米中関係 OR 中東情勢 OR ウクライナ OR 台湾海峡 OR 北朝鮮 OR シリア OR イラン OR イスラエル OR パレスチナ OR ガザ OR 南シナ海 OR インド太平洋 OR ロシア外交 OR 日米同盟 OR 日中関係 OR 日韓関係 OR 軍事 OR 核兵器 OR ミサイル OR 国防 OR 防衛 OR 同盟 OR 有事 OR サミット OR 条約 OR 協定 OR 経済制裁 OR 貿易摩擦 OR 関税 OR 難民 OR 人権 OR 紛争 OR 戦争 OR 停戦 OR 和平&hl=ja&gl=JP&ceid=JP:ja'
...
async def on_ready():
    ...
    try:
        ...
        print('Google News RSSから国際情勢ニュースを取得中...')
        ...
        os.system(f'osascript -e \'display notification "{today}の国際情勢ニュース {posted_count}件をIdiomに投稿しました" with title "Idiom 国際情勢ニュース"\'')
